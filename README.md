...
| ![Administration](docs/images/administration.png) | **Administration.** Accounts, quotas, storage headroom, branding and the installation's one mail server. It can do exactly fourteen things and read nothing sensitive. |
...
Beyond reading it can do exactly fourteen things — set the default report and
...
colour theme, and set, clear or test-send the installation's one SMTP server.
...
That list is a contract, not a summary. A test asserts those fourteen method/path
pairs are handled and that every other write is not, so adding a fifteenth means
