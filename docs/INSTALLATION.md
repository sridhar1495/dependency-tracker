...
| `SCHEDULE_MAX_RESOLVED_PROJECTS` | `250` | The most projects one scheduled run may cover. Only a schedule that stores a **rule** ("latest versions under these", "all latest in the portfolio") can reach it: a rule grows on its own as branches are added under its anchors, with nobody editing the schedule. A run over the limit is **refused, never truncated** — a workbook quietly covering the first 250 of 400 projects says nothing on its face about the 150 it left out |
